...
export function KpiDetailDrawer({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end bg-black/50" onClick={onClose}>
      <aside
        className="flex h-full w-full max-w-[440px] flex-col border-l border-[var(--line)] bg-[var(--panel)] shadow-2xl"
      ...
      >
        <header className="flex items-start justify-between border-b border-[var(--line)] px-5 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded border border-[var(--line)] px-2 py-1 text-[11px] text-[var(--muted)] hover:text-[var(--text)]"
          >
            Close
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {detailQuery.isLoading || !kpi ? (
            ...
            <>
              ...
              <div className="mb-5 h-[160px] rounded border border-[var(--line)] bg-[var(--panel-2)] p-2">
                {histLoading ? (
...
function Tile({
  ...
  return (
    <div className={`rounded border border-[var(--line)] bg-[var(--panel-2)] ${compact ? "px-2 py-1.5" : "px-3 py-2"}`}>
      <div className="text-[9px] uppercase tracking-[0.08em] text-[var(--muted-2)]">{label}</div>
      <div className={`font-mono ${compact ? "text-[12px]" : "text-[14px]"} font-semibold`} style={{ color: accent }}>
        {value}
      </div>
    </div>
  );
